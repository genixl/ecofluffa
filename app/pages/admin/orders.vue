<template>
  <div>
    ...
    <div v-else-if="orders.length === 0" class="bg-surface border border-theme rounded-xl p-10 text-center text-muted">
      No orders yet.
    </div>
      ...
      <table class="w-full text-left">
        ...
        <tbody class="divide-y divide-border-theme">
          <tr v-for="o in orders" :key="o.id" class="hover:bg-subtle transition-colors">
            ...
            <td class="p-4">
              <NuxtLink :to="`/admin/order/${o.id}`" class="font-semibold text-sm hover:underline" style="color: var(--brand-blue);">
                Manage →
              </NuxtLink>
            </td>
          </tr>
...
const { orders, loadAll } = useAdminPlatform()
